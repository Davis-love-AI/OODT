...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 15:16:39 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 00:57:53 2019
...
def toInnerBox(box_flatten, 
    ...
    ln = torch.abs(l - pt_w_flatten[:, 0]) + 1
    tn = torch.abs(t - pt_h_flatten[:, 0]) + 1
    rn = torch.abs(r - pt_w_flatten[:, 0]) + 1
    bn = torch.abs(b - pt_h_flatten[:, 0]) + 1
    

    
    reg_targets = torch.stack([ln[:, None], tn[:, None], 
                               rn[:, None], bn[:, None]], dim=1)
    
    return reg_targets


@IENET_HEAD_LO_REGISTRY.register()
class AVODWH_WH_LO(object):
    """
...
#            pt_h_targets[i] = torch.split(pt_h_targets[i], num_points_per_level, dim=0)
...
#            
            ...
            pt_targets_per_im = torch.stack([point_w1, point_h1], dim=2)
            ...
            pt_targets_per_im = pt_targets_per_im[range(len(locations)), locations_to_gt_inds]
            ...
            pt_targets.append(pt_targets_per_im)
    ...
    def __call__(self, locations, prediction, targets):
        ...
        box_regression = prediction["bbox_reg"]
        centerness = prediction["centerness"]
        pt_reg = prediction["pt_reg"]
        
        N = box_cls[0].size(0)
        num_classes = box_cls[0].size(1)
        
        labels, reg_targets, pt_targets = self.prepare_targets(locations, targets)
        ...
        labels_flatten = []
        reg_targets_flatten = []
        pt_regression_flatten = []
        pt_targets_flatten = []
        for l in range(len(labels)):
            box_cls_flatten.append(box_cls[l].permute(0, 2, 3, 1).reshape(-1, num_classes))
            box_regression_flatten.append(box_regression[l].permute(0, 2, 3, 1).reshape(-1, 4))
            pt_regression_flatten.append(pt_reg[l].permute(0, 2, 3, 1).reshape(-1, 2))
            labels_flatten.append(labels[l].reshape(-1))
            reg_targets_flatten.append(reg_targets[l].reshape(-1, 4))
            pt_targets_flatten.append(pt_targets[l].reshape(-1, 2))
            centerness_flatten.append(centerness[l].reshape(-1))

        box_cls_flatten = torch.cat(box_cls_flatten, dim=0)
        box_regression_flatten = torch.cat(box_regression_flatten, dim=0)
        pt_regression_flatten = torch.cat(pt_regression_flatten, dim=0)
        centerness_flatten = torch.cat(centerness_flatten, dim=0)
        labels_flatten = torch.cat(labels_flatten, dim=0)
        reg_targets_flatten = torch.cat(reg_targets_flatten, dim=0)
        pt_targets_flatten = torch.cat(pt_targets_flatten, dim=0)

        pos_inds = torch.nonzero(labels_flatten > 0).squeeze(1)

        box_regression_flatten = box_regression_flatten[pos_inds]
        pt_regression_flatten = pt_regression_flatten[pos_inds]
        reg_targets_flatten = reg_targets_flatten[pos_inds]
        pt_targets_flatten = pt_targets_flatten[pos_inds]
        centerness_flatten = centerness_flatten[pos_inds]
        
#        inner_box_regression = toInnerBox(box_regression_flatten,
#                                          pt_w_regression_flatten, 
#                                          pt_h_regression_flatten)
#        
#        inner_box_target = toInnerBox(reg_targets_flatten, 
#                                      pt_w_targets_flatten, 
#                                      pt_h_targets_flatten)
        ...
        if pos_inds.numel() > 0:
            ...
            reg_loss = self.box_iou_func(
                box_regression_flatten,
                reg_targets_flatten,
                centerness_targets
            ) / sum_centerness_targets_avg_per_gpu
                    
            reg_l1_loss = self.box_size_l1_func(
                box_regression_flatten,
                reg_targets_flatten,
                centerness_targets
            ) / sum_centerness_targets_avg_per_gpu   
                    
#            inner_reg_loss = self.box_reg_mini_loss_func(
#                torch.abs(pt_regression_flatten + 1),
#                torch.abs(pt_targets_flatten + 1)
#            ) / sum_centerness_targets_avg_per_gpu
                    
            pt_loss = self.box_size_l1_func(
                pt_regression_flatten,
                pt_targets_flatten,
                centerness_targets
            ) / sum_centerness_targets_avg_per_gpu   
            ...
            centerness_loss = self.bce_loss_func(
                centerness_flatten,
                centerness_targets
            ) / num_pos_avg_per_gpu
        else:
            reg_loss = box_regression_flatten.sum()
            reg_l1_loss = box_regression_flatten.sum()
#            inner_reg_loss = pt_regression_flatten.sum()
            pt_loss = pt_regression_flatten.sum()
