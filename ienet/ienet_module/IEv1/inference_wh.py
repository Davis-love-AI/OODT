...
class AVODWH_WH_IN(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        Arguments:
            ...
            box_coder (BoxCoder)
        """
        super(AVODWH_WH_IN, self).__init__()
    ...
    def forward_for_single_feature_map(
            self, locations, box_cls, box_regression, centerness, pt_reg,
            image_sizes):
        """
        Arguments:
        ...
        pt_reg = pt_reg.view(N, 2, H, W).permute(0, 2, 3, 1)
        pt_reg = pt_reg.reshape(N, -1, 2)
        ...
        for i in range(N):
            ...
            per_pt_reg = pt_reg[i]
            per_pt_reg = per_pt_reg[per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                per_pt_reg = per_pt_reg[top_k_indices]
...
#        sampled_boxes = []
        ...
        scores = []
        pred_boxes = []
        for _, (l, o, b, c, p) in enumerate(zip(locations, box_cls, box_regression, centerness, pt_reg)):
            sampled_boxes = self.forward_for_single_feature_map(
                    l, o, b, c, p, image_sizes
                )
