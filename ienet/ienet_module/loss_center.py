...
from ..layers import IOULoss, smooth_l1_loss_weight, SigmoidFocalLoss


from .build import IENET_HEAD_LO_REGISTRY
...
#        self.focal_loss_alpha = cfg.MODEL.AVOD.LOSS_GAMMA
#        self.focal_loss_gamma = cfg.MODEL.AVOD.LOSS_ALPHA
...
#        self.box_reg_mini_loss_func = IOULoss(self.iou_loss_type)
        self.box_size_l1_func = smooth_l1_loss_weight()
#        self.pt_h_reg_loss_func = smooth_l1_loss_weight()
...
#        reg_targets_insize = reg_targets_size_half - reg_targets[:, [0, 1]]
#        quarter_w_lable = (reg_targets_insize[:, 0] > 0).float()
#        quarter_h_lable = (reg_targets_insize[:, 1] > 0).float()
#        reg_targets_insize = torch.abs(reg_targets_insize) * 2
...
#        bbox_reg_insize = prediction["bbox_reg_insize"]
        centerness = prediction["center"]
        confs = prediction["confs"]
#        quarters_w = prediction["quarters_w"]
#        quarters_h = prediction["quarters_h"]
...
#            bbox_reg_insize_flatten.append(bbox_reg_insize[l].permute(0, 2, 3, 1).reshape(-1, 2))
            centerness_flatten.append(centerness[l].permute(0, 2, 3, 1).reshape(-1, 2))
            confs_flatten.append(confs[l].reshape(-1))
#            quarter_w_flatten.append(quarters_w[l].reshape(-1))
#            quarter_h_flatten.append(quarters_h[l].reshape(-1))
...
#        quarter_w_flatten = torch.cat(quarter_w_flatten, dim=0)
#        quarter_h_flatten = torch.cat(quarter_h_flatten, dim=0)
        bbox_reg_size_flatten = torch.cat(bbox_reg_size_flatten, dim=0)
#        bbox_reg_insize_flatten = torch.cat(bbox_reg_insize_flatten, dim=0)
...
#        bbox_reg_insize_flatten = bbox_reg_insize_flatten[pos_inds]
#        quarter_w_flatten = quarter_w_flatten[pos_inds]
#        quarter_h_flatten = quarter_h_flatten[pos_inds]
...
#            reduce_sum(quarter_w_flatten.new_tensor([0.0]))
#            quarter_w_loss = quarter_w_flatten.sum()
#            reduce_sum(quarter_h_flatten.new_tensor([0.0]))
#            quarter_h_loss = quarter_h_flatten.sum()

        losses = {
            "loss_cls": cls_loss,
            "reg_iou_loss": reg_iou_loss,
            "reg_size_loss": reg_size_loss * 0.1,
#            "reg_insize_loss": reg_insize_loss * 0.1,
            "center_loss": center_loss,
            "conf_loss": conf_loss,
#            "quarter_w_loss": quarter_w_loss,
#            "quarter_h_loss": quarter_h_loss,
            }
