...
class RotatedBoxes(Boxes):
    ...
    def clip(self, box_size: Boxes.BoxSizeType) -> None:
        """
        ...
        idx = torch.where(torch.abs(self.tensor[:, 4]) <= 1)[0]
        ...
        self.tensor[:, 2] = torch.min(self.tensor[:, 2], x2 - x1)
        self.tensor[:, 3] = torch.min(self.tensor[:, 3], y2 - y1)
